...
def top_titles
  ...
  Movie.select(:title, :score).where("score >= 9")
...
def below_average_years
  ...
  Movie.select("yr, COUNT(id) AS bad_movies").where("score < 5").group("movies.yr").order("bad_movies DESC")
...
def uma_movies
  ...
  Movie.select(:id, :title, :yr).joins(:castings).where("? = ?", "castings.actor_id", "actors")
